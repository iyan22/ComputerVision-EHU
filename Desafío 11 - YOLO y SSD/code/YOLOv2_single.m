...
% Se crea una red YOLOv2
input_size = [128 128 3];
num_classes = width(dog_dataset.Var2);

% Se estima el tamaño de los anchor boxes utilizando los datos de entrenamiento.
training_data_estimation = transform(training_data, @(data)preprocessData(data, input_size));
num_anchors = 3;
[anchor_boxes, mean_IoU] = estimateAnchorBoxes(training_data_estimation, num_anchors);

% Se crea la red resnet50
...
% Se entrena el detector de objetos YOLOv2.
...
path = strcat(ruta_principal, subcarpeta_results, 'YOLOv2');
...
for i=1:size(imds_validation.Files, 1)
    ...
    [bboxes, scores] = detect(detector,I);
    I = insertObjectAnnotation(I, 'Rectangle', bboxes, scores);
    imwrite(I, strcat(path, 'YOLOv2_validation_', num2str(i), '.png'))
end 
...
for i=1:size(imds_test.Files, 1)
    ...
    [bboxes, scores] = detect(detector,I);
    I = insertObjectAnnotation(I, 'Rectangle', bboxes, scores);
    imwrite(I, strcat(path, 'YOLOv2_test_', num2str(i), '.png'))
    if i <= 25
        subplot(5,5,i);
        imshow(I);
    end
